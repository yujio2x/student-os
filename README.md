...
Это ещё не публичная production beta. OIDC login, Stars accounting, общий photo flow и JSON restore реализованы и проверены fixtures; live Telegram/Stars/OCR и production domain требуют ручной staging-проверки. Старые балансы не мигрируются; автоматического слияния аккаунтов нет. В текущую версию не входят история заданий, recurring events, timezone engine, Contacts/Knowledge Base, OCR сканированных PDF, cloud sync и нативные приложения. PWA не хранит пользовательские API-данные офлайн; реальная installability в Chrome должна быть проверена на staging.

Рабочий режим Student AI требует серверного OPENAI_API_KEY в Core, не ключа в браузере или bridge-mode боте. Платные запросы и production deployment в ходе разработки не выполнялись. Restore заменяет только свои занятия, дедлайны и настройки после preview; перед заменой сохраните экспорт.
